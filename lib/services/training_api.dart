...
extension TrainingApiService on ApiService {
  /// 获取训练题目
  ...
  static Future<List<TrainingQuestion>> getTrainingQuestions({
    ...
    final response = await get('/training/questions?scope=$scope&mode=$mode&count=$count');
  ...
  static Future<Map<String, dynamic>> submitTrainingResult({
    ...
    return await post('/training/result', body: {
  ...
  static Future<List<Map<String, dynamic>>> getTrainingHistory({
    ...
    final response = await get('/training/history?page=$page&page_size=$pageSize');
